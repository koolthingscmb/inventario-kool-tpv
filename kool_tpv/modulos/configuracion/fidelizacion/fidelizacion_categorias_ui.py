...
class FidelizacionCategoriasUI:
    def __init__(self, parent, db, module_name='config'):
        ...
        try:
            ...
            logging.exception('Error cargando colores en FidelizacionCategoriasUI')
            self.colors = {}
    ...
    def _load_categorias(self):
        """Cargar categorías desde BD y crear chips."""
        if not self.db:
            return
        ...
        try:
            ...
            for i, row in enumerate(rows):
                try:
                    ...
                    # Guardar datos en el botón (categorías usan '_cat_data')
                    setattr(btn, '_cat_data', {
                        'id': item_id,
                        'nombre': nombre,
                    ...
                    logging.exception('Error procesando categoría en chips')

        except Exception:
            logging.exception('Error cargando categorías en chips')
    ...
    def _select_chip(self, btn):
        ...
        try:
            ...
            # Cargar datos
            cat_data = getattr(btn, '_cat_data', None)
            if cat_data:
                self.selected_categoria = cat_data
                porcentaje = cat_data.get('fide_porcentaje', 0)
                self.lbl_valor_actual.configure(text=str(porcentaje))
                logging.debug(f"Categoría seleccionada: {cat_data.get('nombre')}, %: {porcentaje}")

        except Exception:
            logging.exception('Error seleccionando chip')

    def _on_guardar(self):
        """Guardar nuevo % para categoría seleccionada."""
        if not self.db:
            return
        ...
        try:
            self.fidel_repo.actualizar_fidelizacion_categoria(
                self.selected_categoria['id'],
                float(nuevo_valor)
            )
