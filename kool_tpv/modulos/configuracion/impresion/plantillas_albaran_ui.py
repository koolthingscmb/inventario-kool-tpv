"""
PlantillasAlbaranUI: configuración de plantilla visual para exportación PDF de albaranes.

Permite definir colores, título y opciones del PDF desde la UI,
guardando en la tabla `configuracion` con claves `albaran_pdf_*`.
"""
import logging
import shutil
...
CLAVES_PLANTILLA = {
    'albaran_pdf_titulo':            'ALBARÁN DE ENTRADA',
    'albaran_pdf_color_primario':    '#1F6AA5',
    'albaran_pdf_color_secundario':  '#4A90A4',
    'albaran_pdf_mostrar_tienda':    '1',
    'albaran_pdf_mostrar_logo':      '0',
    'logo_pdf_filename':             '',
}


class PlantillasAlbaranUI:
    def __init__(self, parent, db, module_name='config'):
        ...
        try:
            ...
            logging.exception('Error cargando colores en PlantillasAlbaranUI')
            self.colors = {}
        ...
        ctk.CTkLabel(
            self.container,
            text='Configura el aspecto visual de los PDFs exportados desde Almacén → EXPORTAR.',
        ...
        ctk.CTkLabel(
            self.fields_frame, text='Título del documento',
        ...
        # Fila 1: Color primario
        ctk.CTkLabel(
            self.fields_frame, text='Color primario (hex)',
            font=lbl_font, text_color=self.colors.get('text')
        ...
        # Fila 2: Color secundario
        ctk.CTkLabel(
            self.fields_frame, text='Color secundario (hex)',
            font=lbl_font, text_color=self.colors.get('text')
        ...
        # Fila 3: Mostrar datos de tienda
        ctk.CTkLabel(
            self.fields_frame, text='Mostrar datos de tienda',
            font=lbl_font, text_color=self.colors.get('text')
        ).grid(row=3, column=0, sticky='w', padx=8, pady=8)
        self.var_mostrar_tienda = ctk.StringVar(value='1')
        self.switch_tienda = ctk.CTkSwitch(
            self.fields_frame,
            text='',
            variable=self.var_mostrar_tienda,
            onvalue='1', offvalue='0',
            progress_color=self.colors.get('primary'),
            button_color=self.colors.get('secondary'),
        )
        self.switch_tienda.grid(row=3, column=1, sticky='w', padx=8, pady=8)

        # Fila 4: Mostrar logo
        ctk.CTkLabel(
            self.fields_frame, text='Mostrar logo',
            font=lbl_font, text_color=self.colors.get('text')
        ...
        self.switch_logo.grid(row=4, column=1, sticky='w', padx=8, pady=8)

        # Fila 5: Selector de logo
        ctk.CTkLabel(
            self.fields_frame, text='Logo PDF',
            font=lbl_font, text_color=self.colors.get('text')
        ).grid(row=5, column=0, sticky='w', padx=8, pady=8)

        logo_row = ctk.CTkFrame(self.fields_frame, fg_color='transparent')
        logo_row.grid(row=5, column=1, columnspan=3, sticky='we', padx=8, pady=8)
        ...
        self.lbl_logo_preview.grid(row=6, column=1, columnspan=2, sticky='w', padx=8, pady=4)
    ...
    def _cargar_valores(self):
        """Leer claves de la BD y rellenar campos."""
        try:
            cfg = self.config_repo.obtener_multiples(list(CLAVES_PLANTILLA.keys()))
        except Exception:
            logging.exception('Error leyendo plantilla desde BD')
            cfg = {}
        ...
        self.e_titulo.insert(0, _val('albaran_pdf_titulo'))

        self.e_color_primario.delete(0, 'end')
        self.e_color_primario.insert(0, _val('albaran_pdf_color_primario'))

        self.e_color_secundario.delete(0, 'end')
        self.e_color_secundario.insert(0, _val('albaran_pdf_color_secundario'))

        self.var_mostrar_tienda.set(_val('albaran_pdf_mostrar_tienda'))
        self.var_mostrar_logo.set(_val('albaran_pdf_mostrar_logo'))
    ...
    def _on_seleccionar_logo(self):
        """Abrir selector de imagen y copiar a assets/."""
        try:
    ...
    def _on_quitar_logo(self):
        """Eliminar referencia al logo en BD (no borra el archivo)."""
        try:
            self.config_repo.guardar_multiples({'logo_pdf_filename': ''})
            self.lbl_logo_path.configure(text='Sin logo seleccionado')
            self.lbl_logo_preview.configure(image=None, text='')
        except Exception:
            logging.exception('Error quitando logo PDF')

    def _mostrar_preview_logo(self, path):
        """Mostrar miniatura del logo en la UI."""
        try:
            img = Image.open(path)
            ...
            logging.exception('Error mostrando preview de logo')
            self.lbl_logo_preview.configure(image=None, text='(preview no disponible)')

    def _on_guardar(self):
        """Guardar campos en la BD."""
        campos = {
            'albaran_pdf_titulo':           self.e_titulo.get().strip(),
            'albaran_pdf_color_primario':   self.e_color_primario.get().strip(),
            'albaran_pdf_color_secundario': self.e_color_secundario.get().strip(),
            'albaran_pdf_mostrar_tienda':   self.var_mostrar_tienda.get(),
            'albaran_pdf_mostrar_logo':     self.var_mostrar_logo.get(),
        }
        try:
    ...
    def _on_restaurar(self):
        ...
        try:
            ...
            logging.exception('Error restaurando plantilla por defecto')
