"""Prompts de generación de contenido para camisetas.

Transcripción EXACTA de los prompts del Google Apps Script original.
Los textos se conservan palabra por palabra; las interpolaciones JS (${...})
se convierten en placeholders Python ({...}) que se rellenan con str.replace
para no interferir con las llaves {} del ejemplo JSON del prompt de body.
"""

# Marcadores disponibles en PROMPT_SEO_DESCRIPCION:
#   {titulo_base}  {tags}  {beneficio}
...
#   {genero}  {titulo_base}  {tono}  {instrucciones_genero}  {tags}  {beneficio}  {tags_top3}
...
ESTRUCTURA DEL JSON A DEVOLVER:
{
  ...
  "bloque_calidad_algodon": "[Describe la sensación de suavidad del algodón premium.]",
...
# Instrucciones por género (texto literal del GAS, se inyecta en {instrucciones_genero})
...
Usa frases como: 'Tu hijo/a lucirá increíble', 'Regala a tu pequeño/a', 'La camiseta perfecta para que tu hijo/a muestre su pasión'.
...
# Prompt NUEVO (no existe en el GAS): generación de tags.
# Marcadores: {titulo_base}
PROMPT_TAGS = """Genera una lista de 8-12 tags para Shopify, en español, separados por comas, para una camiseta sobre "{titulo_base}".
...
# Plantilla HTML del body — transcripción EXACTA del GAS (incluye el </ul>
# duplicado original; Shopify lo normaliza igual que hacía con el script).
# Marcadores: {titulo_introductorio} {parrafo_introductorio} {titulo_seccion_calidad}
# {bloque_calidad_impresion} {bloque_calidad_algodon} {bloque_calidad_durabilidad}
# {bloque_porque_elegirnos} {corte} {link_guia} {botones_html}
...
<ul>
  ...
  <li><strong>Algodón Premium 100%:</strong> {bloque_calidad_algodon}</li>
...
# Patrón del SEO title (fijo en el GAS, sin IA).
# Marcadores: {titulo} {genero} {marca}
PLANTILLA_SEO_TITLE = "{titulo} | {genero} | {marca}"

# Botón de enlace a otro género (el GAS lo construía por género restante).
...
# Prompt por defecto del flujo manga (BUSCAR DATA). El texto real puede ya
# existir editado en configuracion (clave shopify_ia_seo_prompt); la migración
# lo respeta y solo usa este texto si no hay ninguno guardado.
