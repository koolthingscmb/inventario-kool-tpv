...
class FuentesTab:
    ...
    def render(self):
        ...
        selection_zone = tk.Frame(scroll, bg=self._bg_color)
        ...
        for source in sources:
            f_frame = tk.Frame(scroll, bg=self._bg_medium, bd=1, relief="flat")
    ...
    def _render_source_tags(self, source, parent):
        ...
        try:
            query = """
                ...
                WHERE m.source_id = ? ORDER BY t.nombre ASC
            """
    ...
    def _on_add_type_to_source(self, source, tags_frame):
        if not hasattr(self, 'type_combo'): return
