...
class VentaTicketGenerator(BaseTicketGenerator):
    ...
    def generate(self, config, ticket_data, items, cliente_data=None):
        ...
        # Tesoro gastado si aplica (mostrar etiqueta según motivo: 'gasto'|'devolucion')
        tesoro = ticket_data.get('tesoro_data') or {}
        tesoro_gastado = tesoro.get('gastado', 0) if isinstance(tesoro, dict) else 0
        motivo = ''
        try:
            motivo = (tesoro.get('motivo') or '') if isinstance(tesoro, dict) else ''
        except Exception:
            motivo = ''
        try:
            if Decimal(str(tesoro_gastado)) > 0:
                val_s = f"-{self._format_currency(tesoro_gastado)}"
                motivo_norm = str(motivo).strip().lower() if motivo is not None else ''
                if motivo_norm == 'devolucion':
                    label = '>> TESORO ANULADO <<'
                elif motivo_norm == 'gasto':
                    label = '>> TESORO CANJEADO <<'
                else:
                    # Fallback: conservar comportamiento anterior
                    label = '>> TESORO CANJEADO <<'

                # Alineamos etiqueta a la izquierda y el valor a la derecha
                line = label.ljust(self.WIDTH - len(val_s)) + val_s
                lines.append(line)
        except Exception:
            pass
        ...
        if cliente_data:
            ...
            motivo_tesoro = ''
            try:
                motivo_tesoro = (ticket_data.get('tesoro_data', {}).get('motivo') or '').strip().lower()
            except Exception:
                motivo_tesoro = ''
