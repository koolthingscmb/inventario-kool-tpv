...
class VarianteProductoRepository:
    ...
    def _row_to_link(self, row) -> VarianteProductoLink:
        ...
        try:
            ...
            # id, v_id, p_id, ratio, activo, created_at, updated_at
            id_, v_id, p_id, ratio, activo, created_at, updated_at = row[:7]
            v_nombre = row[7] if len(row) > 7 else None
            p_nombre = row[8] if len(row) > 8 else None
    ...
    def get_todos(self) -> List[VarianteProductoLink]:
        """Obtener todos los mapeos con nombres de variante y producto."""
        query = """
            SELECT l.id, l.variante_id, l.producto_id, l.ratio, l.activo, l.created_at, l.updated_at,
    ...
    def crear(self, link: VarianteProductoLink) -> Optional[int]:
        ...
        try:
            query = """
                INSERT INTO produccion_variantes_productos (variante_id, producto_id, ratio, activo)
                VALUES (?, ?, ?, ?)
            """
            self.db.execute_query(query, (link.variante_id, link.producto_id, link.ratio, link.activo))
    ...
    def actualizar(self, link: VarianteProductoLink) -> bool:
        ...
        try:
            query = """
                ...
                SET variante_id = ?, producto_id = ?, ratio = ?, activo = ?, updated_at = CURRENT_TIMESTAMP
                WHERE id = ?
            """
            self.db.execute_query(query, (link.variante_id, link.producto_id, link.ratio, link.activo, link.id))
            return True
        except Exception:
