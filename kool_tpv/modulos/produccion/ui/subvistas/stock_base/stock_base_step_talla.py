"""Paso 5: Selección de talla (XL, L, M, etc.) con chips."""
...
class StockBaseStepTalla(KeyboardNavigableMixin):
    """Subvista para seleccionar talla con chips.

    Args:
        parent: Widget padre.
        on_siguiente: Callback cuando se selecciona talla (recibe str).
        on_volver: Callback para volver al paso anterior.
        tallas_disponibles: Lista de dicts con 'codigo' y 'nombre'.
    """
    ...
    def __init__(self, parent,
        ...
        self.tallas = tallas_disponibles or []
    ...
    def _crear_chips_tallas(self):
        ...
        chip_font = (font_family[0], default_style.get("font_size", 84), font_family[2])
    ...
    def _on_nav_enter_callback(self, btn: ctk.CTkButton, talla_codigo: Optional[str]):
        if self._selected_chip is not None and self._selected_chip == btn:
            if self.on_siguiente and self.talla_seleccionada:
