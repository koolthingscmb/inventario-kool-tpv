...
class StockSubView(CTkFrame):
    ...
    def __init__(self, parent, db, carrito_service, view=None):
        ...
        # Config local para layout: permitir wraplength para nombres largos
        layout_config = {
            "wraplength": 300
        }
        ...
        # Asegurar que el NavList interno recibe el wraplength (compatibilidad)
        try:
            if getattr(self.search_list, 'nav_list', None):
                self.search_list.nav_list.wraplength = layout_config.get("wraplength")
        except Exception:
            pass
    ...
    def _on_producto_double_click(self, data):
        try:
            # data may be the mapped row dict or None; prefer to use selected data
            if not data:
