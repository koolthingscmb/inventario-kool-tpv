    ...
    def get_resumen_financiero(self) -> Dict:
        ...
        try:
            ...
            if total_descuento_bruto > Decimal('0.00'):
                ...
                if total_bruto_original <= Decimal('0.00'):
                    for tipo in iva_items.keys():
                    ...
                    for tipo, cuota in iva_items.items():
                        ...
                        try:
                            base_orig = cuota / (tipo_pct / Decimal('100'))
                        except Exception:
                    ...
                    for tipo, gross_orig in gross_by_type.items():
                        try:
                            proporcion = (gross_orig / total_bruto_original) if total_bruto_original > Decimal('0.00') else Decimal('0.00')
                            descuento_para_tipo = (total_descuento_bruto * proporcion)
                            nueva_gross = gross_orig - descuento_para_tipo
                            if nueva_gross < Decimal('0.00'):
                                nueva_gross = Decimal('0.00')
                            tipo_pct = Decimal(tipo)
