...
# --- WIDGET PRINCIPAL ---
class BuscarArticuloWidget(ctk.CTkFrame, KeyboardNavigableMixin):
    """Subvista de búsqueda de artículos. Muestra categorías/tipos y productos."""
    ...
    def __init__(self, parent, db, carrito_service, on_add_callback=None, on_close_callback=None, **kwargs):
        ...
        # Configurar navegación por teclado (bindings directos, es subvista única)
    ...
    def cambiar_modo(self, nuevo_modo):
        # Limpiar navegación antes de reconstruir
        ...
        # Reconfigurar navegación después de cambiar modo
        self._setup_buscar_keyboard_nav()
    ...
    def seleccionar_categoria(self, nombre):
        ...
        # Reconfigurar navegación después de mostrar productos
        self._setup_buscar_keyboard_nav()

    def _setup_buscar_keyboard_nav(self):
        """Configurar navegación por teclado para todos los botones del buscador."""
        self._navigable_buttons = []
        
        # 1. Botones de modo (CATEGORÍAS, TIPOS)
        ...
        # 2. Botones de categorías/tipos en cat_scroll
        ...
        # 3. Botones de productos en prod_scroll
        for child in self.prod_scroll.winfo_children():
            if isinstance(child, ctk.CTkButton):
                wrapped = lambda c=child: self._execute_btn_command(c)
                self._navigable_buttons.append((child, wrapped))
        
        # Activar navegación con bindings en toplevel (como CajeroSubView)
    ...
    def _ensure_button_visible(self, btn):
        """Hacer scroll para que un botón sea visible."""
        try:
            # Determinar en qué scroll está el botón
            parent = btn.winfo_parent()
            if str(self.cat_scroll) in str(parent):
                scroll = self.cat_scroll
            elif str(self.prod_scroll) in str(parent):
                scroll = self.prod_scroll
            else:
                return
            ...
            y = btn.winfo_y()
            scroll._parent_canvas.yview_moveto(y / scroll._parent_canvas.bbox("all")[3])
        except Exception:
            pass
        ...
        if self.carrito_service:
            ...
            try:
                ...
                if tpv_view and hasattr(tpv_view.controller, 'handle_add_product'):
                    tpv_view.controller.handle_add_product(producto_para_carrito)
